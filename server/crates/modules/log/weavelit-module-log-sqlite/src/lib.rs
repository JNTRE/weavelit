...
impl SqliteLogDestination {
    ...
    pub fn open(context: &TrustedLogModuleContext) -> Result<Self, LogDestinationError> {
        ...
        destination.apply_migrations()?;
        destination.bind_deployment(context.deployment_identity())?;
        Ok(destination)
    }
    ...
    fn apply_migrations(&mut self) -> Result<(), LogDestinationError> {
        validate_registry()?;
        let connection = self
            .connection
            .get_mut()
    ...
    fn bind_deployment(&self, deployment_identity: &[u8; 16]) -> Result<(), LogDestinationError> {
        ...
        match bindings.as_slice() {
            [] => {
                transaction
                    .execute(
                        &format!("INSERT INTO {DEPLOYMENT_TABLE} (singleton, deployment_identity) VALUES (1, ?1)"),
                        [deployment_identity.as_slice()],
                    )
                    .map_err(|_| LogDestinationError::IntegrityFailure)?;
            }
            [binding] if binding.as_slice() == deployment_identity => {}
            _ => return Err(LogDestinationError::IntegrityFailure),
        }
