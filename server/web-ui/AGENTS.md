...
- `src/`: TypeScript and React application source and its unit tests, organized into `api/` (status, Application Database selection, Restore submission, and authentication transport clients), `components/` (application shell, Restore submission form, and sign-in form), `hooks/` (deployment status hook), and `styles/` (application stylesheet), following the `weavelit-<phase>-<component>` file-naming convention.
