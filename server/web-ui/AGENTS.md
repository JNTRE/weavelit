...
- `browser-tests/`: Playwright browser tests, covering the pre-operational status smoke test, the Application Database selection restart-persistence test, and the two-request Restore submission test, and the shared fixture that runs them against the release Server binary.
...
- `src/`: TypeScript and React application source and its unit tests, organized into `api/` (status, Application Database selection, and Restore submission transport clients), `components/` (application shell and Restore submission form), `hooks/` (deployment status hook), and `styles/` (application stylesheet), following the `weavelit-<phase>-<component>` file-naming convention.
