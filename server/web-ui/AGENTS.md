...
- `src/`: TypeScript and React application source and its unit tests, organized into `api/` (status, Application Database selection, Init setup and recovery-key proof-of-possession, Restore submission, authentication, and account-administration transport clients), `components/` (application shell, Init workflow, Restore submission form, sign-in form, and authenticated Accounts workspace), `hooks/` (deployment status hook), and `styles/` (application stylesheet), following the `weavelit-<phase>-<component>` file-naming convention.
