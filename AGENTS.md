...
- `Weavelit CLI source tree`: Source, tests, and macOS release packaging for the separately packaged client; follow its local guide.
...
- For changes under `server/` or in the Weavelit CLI source tree, read the matching local guide and the canonical documentation it routes to before editing.
