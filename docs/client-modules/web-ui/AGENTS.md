...
- Keep the Web UI Client Module's Init-capable and Restore-capable transport
  behavior here. `../../server/lifecycle-design.md` owns availability and
  database selection, `../../server/init-design.md` owns fresh-state secrets and
  recovery-key delivery, and `../../server/restore-design.md` owns backup and
