# Milestone 11: Build Support for Automation Identities

## Goals

- [ ] An **[Administrator](../../glossary.md#identities-and-access)** can create and manage an **[Automation Identity](../../glossary.md#identities-and-access)**, including its credentials and named **[Operation](../../glossary.md#applications-and-interfaces)** scopes.
- [ ] Each Automation Identity has an active **[Responsible Owner](../../glossary.md#identities-and-access)** who is a **[Human User](../../glossary.md#identities-and-access)** but cannot change the Automation Identity's permissions or credentials through ownership alone.

## Related Documents

- [Vision](../../vision.md)
- [Core Statements](../../core-statements.md)
- [Security Model](../../security-model.md)
- [Glossary](../../glossary.md)
- [Open Questions](../../open-questions.md)
