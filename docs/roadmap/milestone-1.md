...
- [ ] A **[Host Administrator](../glossary.md#identities-and-access)** can use
  the **[Admin CLI](../glossary.md#applications-and-interfaces)** to complete
  **[Init](../glossary.md#states-and-requests)** interactively or from an
  explicit non-interactive bootstrap configuration file, create the
...
- [ ] A Host Administrator can use the Admin CLI to set the Weavelit Server
...
- [ ] The Server evaluates each local Human User's MFA policy, enabled
  **[MFA Modules](../glossary.md#applications-and-interfaces)**, and verified
  MFA factors before granting a usable session. A Human User whose account
  requires MFA cannot obtain a usable session until they verify an enabled MFA
  method.
