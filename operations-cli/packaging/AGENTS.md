# Operations CLI Packaging Agent Guide

This directory is reserved for assets that package the separately released
Operations CLI. The release artifact installs the client without Rust, source
...
- This directory owns Operations CLI release-artifact boundaries.
- It does not own Operations CLI source, Server packaging, provider credentials, or Server initialization.
...
- `AGENTS.md`: Local routing, inventory, and Operations CLI packaging-boundary rules.
- `macos/`: macOS `arm64` Operations CLI packaging boundary.
...
- Read `../../docs/roadmap/milestone-8.md` and the Operations CLI requirements before changing release-artifact behavior.
...
- Package the Operations CLI independently from the Server while respecting the versioned interface compatibility policy.
- Do not include Rust, source code, or provider credentials in an installed Operations CLI artifact.
